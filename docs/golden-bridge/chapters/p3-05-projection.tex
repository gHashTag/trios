...
Figure (fig-p3-ch5-projection). Projection triptych --- fine lattice / coarse lattice / pi-L bridge
  ...
  5.3 Composing Pellis and Trinity Representations
        ...
        Theorem 5.6 (Composite expansion; Theorem). Let T^* = PiL(x) and r =
        RL(x). Then

  x = T^* + sumk=0D ck res $\varphi$-k + O($\varphi$-(D+1)), 2
  ...
  skeleton) + Pellis series (fine-scale additive correction).
        ...
        Definition 5.7 (Unification representation). The pair (T^*, (ck res)k=0D) is
  ...
  relative to the Trinity approximant.
        ...
        and any T^ $\in$ T cap Z[$\varphi$], the residual RL(x) = x - T^ $\in$ Z[$\varphi$], and its Pellis
