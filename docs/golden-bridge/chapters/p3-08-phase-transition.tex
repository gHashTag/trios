...
8. The Symbolic Phase Transition Conjecture
  ...
  $\varepsilon$^, Trinity monomials suffice; above $\varepsilon$^, the Pellis series adds essential information.
  ...
  muT(x) = limsupL $\rightarrow$ $\in$fty (log |RL(x)|)/(ell(lambda^*L)) $\cdot$ (1)/(log $\varphi$-1).

  There exists a critical exponent mu^* = 1 such that:

  - Pellis-dominant phase (muT(x) > mu^*): Trinity monomials approximate                                 x
  sub-optimally per bit; Pellis corrections are necessary at all precisions.
  - Trinity-dominant phase (muT(x) < mu^*): x lies near a Trinity attractor,
  well-approximated by low-complexity monomials.

  - Critical line (muT(x) = mu^*): scale-free approximation behaviour; the Trinity and
  Pellis representations are equally efficient per bit.

  The conjecture asserts that this trichotomy is well-defined and that mu^* = 1 is the
  ...
  8.3 Lucas Ring Elements Are Trinity-Dominant
        ...
        Proof. By Proposition 5.9, RL(x) = 0 exactly when T^* = x $\in$ T. Hence |RL(x)| = 0
