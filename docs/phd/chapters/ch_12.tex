...
\section*{Hardware Bridge (deferred)}
\addcontentsline{toc}{section}{Hardware Bridge (deferred)}
...
\subsection{2. Bridge Architecture and Interface Contracts}\label{bridge-architecture-and-interface-contracts}

\subsubsection{2.1 Logical Structure}\label{logical-structure}
...
\subsubsection{2.2 Signal Naming Convention}\label{signal-naming-convention}
...
\subsubsection{2.3 Error-Handling Protocol}\label{error-handling-protocol}
...
\item
  \textbf{CDC-SLIP}: if the two-flip-flop synchroniser detects a doubled transition (metastability indicator), the bridge logs the event in a 32-bit saturating counter accessible via \texttt{GS\_CTRL\_CDC\_SLIP}.
\end{itemize}

These conditions are reported to the IGLA RACE monitor (Ch.24) via a 3-bit interrupt line, one bit per error class {[}6{]}.

\subsection{3. Clock-Domain Analysis and Timing}\label{clock-domain-analysis-and-timing}

\subsubsection{3.1 Frequency Ratios and the Golden Ratio}\label{frequency-ratios-and-the-golden-ratio}
...
\subsubsection{3.2 Throughput Budget}\label{throughput-budget}
...
\subsubsection{3.3 Power Accounting}\label{power-accounting}
...
{[}4{]} \texttt{gHashTag/trios\#393} --- Ch.12 Hardware Bridge scope issue.
...
{[}8{]} \texttt{gHashTag/t27/proofs/canonical/} --- Coq canonical proof archive, 65 \texttt{.v} files, 297 Qed.
