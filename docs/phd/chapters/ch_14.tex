...
\section*{Eval Semantics: The BPB Metric}
\addcontentsline{toc}{section}{Eval Semantics: The BPB Metric}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,keepaspectratio]{ch14-eval-semantics.png}
...
\subsection{2. BPB: Definition and Algebraic Properties}\label{bpb-definition-and-algebraic-properties}

\subsubsection{2.1 Cross-Entropy and Perplexity}\label{cross-entropy-and-perplexity}
...
\subsubsection{2.2 Byte-Level Normalisation}\label{byte-level-normalisation}
...
\subsubsection{2.3 φ-Weighted BPB}\label{ux3c6-weighted-bpb}
...
\subsection{3. Gate Thresholds and Their Derivation}\label{gate-thresholds-and-their-derivation}

\subsubsection{3.1 Gate-2: BPB ≤ 1.85}\label{gate-2-bpb-1.85}
...
\subsubsection{3.2 Gate-3: BPB ≤ 1.50}\label{gate-3-bpb-1.50}
...
\subsubsection{3.3 Relationship to the DARPA Energy Goal}\label{relationship-to-the-darpa-energy-goal}
...
{[}12{]} Trinity Canonical Coq Home. \texttt{gHashTag/t27/proofs/canonical/} --- 65 \texttt{.v} files, 297 Qed, 438 total theorems. GitHub repository.
