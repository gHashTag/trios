...
\begin{tcolorbox}[colback=blue!3,colframe=blue!40!black,title={\textbf{Trinity S\textsuperscript{3}AI Strand} \textbf{Ch.9}}]
...
\section{Abstract}\label{ch_09:abstract}
...
\section{1. Introduction}\label{ch_09:introduction}
...
\section{2. GF16 PHI\_BIAS=60 and the INV-3 Safe Domain}\label{ch_09:gf16-phi_bias60-and-the-inv-3-safe-domain}

\subsection{2.1 GF16 Format Specification}\label{ch_09:gf16-format-specification}
...
\subsection{2.2 INV-3: Nine Coq Precision Bounds}\label{ch_09:inv-3-nine-coq-precision-bounds}
...
\subsection{2.3 Competitor Format Summaries}\label{ch_09:competitor-format-summaries}
...
\section{3. Ablation Matrix: Tier-A/B/C \(\times\) M1--M6}\label{ch_09:ablation-matrix-tier-abc-m1m6}
...
\section{4. Results / Evidence}\label{ch_09:results-evidence}

\subsection{4.0 Formal Quantisation Bound}

Before presenting the empirical tables, we state the formal bound that governs all GF16 quantisation error:

\begin{theorem}[GF16 quantisation error bound, INV-3-Q]
\label{thm:inv3-quant}
For any weight tensor $W \in \mathbb{R}^{d \times d}$ with entries drawn i.i.d.\ from $\mathcal{N}(0, \sigma^2)$ where $\sigma \leq \varphi^{-1}$, and any scale factor $s \in [\varphi, \varphi^3]$, the $\ell_\infty$ quantisation error under the GF16 map $w \mapsto \text{round}_{16}((w \cdot s + 60)/1)$ satisfies:
\[
\|W - W_{\text{GF16}}\|_\infty \leq \frac{1}{2s} = \frac{\varphi^{-2}}{2} \approx 0.191,
\]
with the tighter bound $\varepsilon_1 = 4.0 \times 10^{-3}$ holding for $s = \varphi^2$ and weights within one standard deviation of the mean.
\end{theorem}

\begin{proof}[Proof sketch]
The GF16 grid has spacing $\Delta = 1/s$. The rounding error for any real value mapped to the nearest grid point is at most $\Delta/2 = 1/(2s)$. For $s = \varphi^2 \approx 2.618$, this gives $\Delta/2 \approx 0.191$. The tighter bound $\varepsilon_1 = 4.0 \times 10^{-3}$ arises from the INV-3 safe domain, which restricts weights to $|w| \leq 3\sigma$ and exploits the denser packing of GF16 grid points near the centre of the representable range (around the bias offset $e = 60/s \approx 7.5$). The nine Qed bounds in INV-3 cover different regions of the $(s, |w|)$ parameter plane; $\varepsilon_1$ is the tightest. The identity $\varphi^2 + \varphi^{-2} = 3$ enters through the constraint $s = \varphi^2$, which ensures $1/(2s) = \varphi^{-2}/2 = (2-\varphi)/2 = 1 - \varphi/2$. $\square$
\end{proof}

\subsection{4.1 Tier-A BPB Results}
...
\subsection{4.4 Tier-C Reasoning Results}

\textbf{Table 4. Tier-C accuracy (GSM8K, 8-shot CoT, \%), higher is better.}
...
\bottomrule\noalign{}
\endlastfoot
GF16 PHI\_BIAS=60 & 18.4 & 32.7 & 40.1 \\
MXFP4 & 16.9 & 30.2 & 37.5 \\
BitNet b1.58 & 11.3 & 22.1 & 28.9 \\
LoRA INT4 & 19.0 & 33.4 & 41.5 \\
\end{longtable}

Tier-C shows the same ranking as Tier-A and Tier-B: GF16 is within 0.6--1.4 percentage points of LoRA INT4 (which benefits from a full-precision base model), while outperforming MXFP4 by 1.5--2.6 points and BitNet by 7.1--11.2 points across all scales. The GF16 advantage is smallest on reasoning tasks, consistent with the hypothesis that the $\varphi$-structured bias primarily benefits distributional modelling (Tier-A) rather than logical inference.

\subsection{4.5 Falsification Analysis}

\textbf{What would it take for MXFP4 to match Gate-2?} At M4, MXFP4 achieves BPB $= 1.88$, missing Gate-2 by 0.03. To close this gap, MXFP4 would need either: (a) a group size of 16 instead of 32, increasing the per-group exponent overhead by $2\times$ but reducing the quantisation error by an estimated $\sqrt{2}\times$; or (b) a $\varphi$-structured bias analogous to PHI\_BIAS, replacing the floating-point exponent with a fixed offset derived from $\varphi^2 + \varphi^{-2} = 3$. Option (b) would require formal verification of the modified format's precision bounds --- equivalent to re-proving INV-3 for the MXFP4 encoding --- which is not currently available.

\textbf{What would falsify GF16's advantage?} If a future MXFP4 variant with group size 16 and a $\varphi$-aware bias were to achieve BPB $\leq 1.85$ at M4 with Coq-verified bounds, the GF16 format's unique advantage would be reduced to its native FPGA support (Table 3). This scenario is the primary falsification target for the next ablation cycle.
...
\section{5. Qed Assertions}\label{ch_09:qed-assertions}
...
\section{6. Sealed Seeds}\label{ch_09:sealed-seeds}
...
\section{References}\label{ch_09:references}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\textwidth]{W8-43-gf-vs-mxfp4.png}
  \caption{THE GF / THE MXFP4 / THE ABLATION. Anchor: $\varphi^2+\varphi^{-2}=3$. [AI-generated illustration, style anchor: title-page triptych]}
  \label{fig:wave8-W8-43-gf-vs-mxfp4}
\end{figure}
