...
% Phase D). Matches the convention installed for L23 in commit cf1cc90. The
% macro renders the Coq theorem name verbatim in chapter prose; when the
% monograph-wide macro lands in main.tex (post-#288 / LF restoration) the
% \providecommand below silently yields to it.
...
% R14 — Coq citation map (INV-12, Trinity Anchor at vertex-norm level).
% Mirrored from assertions/igla_assertions.json; consumed by appendix F
% (auditor lane LC). Status: Proven byte-for-byte in trinity-clara.
\noindent\textbf{Coq Citation Map (LC · R14):} the INV-12 invariant
referenced throughout this chapter (vertex-norm constraint at lines 420,
435, 786, 1286, 1427, 1505) is mechanised in
\texttt{trinity-clara/proofs/igla/asha\_rungs\_trinity.v} as
\citetheorem{asha_rungs_trinity} (status \texttt{Proven}). The Trinity
Anchor $\phi^{2}+\phi^{-2}=3$ underwriting the great stellated
dodecahedron's vertex-norm corollary (Cor.~\ref{cor:15-anchor}) factors
through the same invariant, since INV-12 enforces the φ-derived
vertex-norm calibration that the Kepler--Poinsot stellation extends.
This chapter's classical Theorem~\ref{thm:15-euler} (Generalised Euler)
is honestly flagged \admittedbox{thm:15-euler} above with a chapter-side
\texttt{Admitted} pending an external Coq port; the geometric INV-12
anchor itself, however, is \texttt{Proven} byte-for-byte in
\texttt{trinity-clara/proofs/igla/asha\_rungs\_trinity.v}, and that is
the only invariant this chapter depends on for runtime mirroring (R14).
