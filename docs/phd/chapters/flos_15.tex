...
\begin{tcolorbox}[colback=yellow!4,colframe=yellow!50!brown,title={\textbf{Flos Aureus} \textbf{FA.15 Kepler Solids}}]
  \textbf{Petal:} Part V --- \emph{Sacred Geometry} \\
  \textbf{Anchor:} \(\varphi^{2} + \varphi^{-2} = 3\) (Trinity Identity, INV-22) \\
  \textbf{Motif:} the four star polyhedra \\
  \textbf{Lane:} L15 (Flos Aureus strand) \\
...
\section{Abstract}\label{fa_15:abstract}
...
\section{1. Introduction}\label{fa_15:introduction}
...
Invariant
(INV-1)}\label{fa_15:bpb-protocol-and-monotone-backward-invariant-inv-1}

\subsection{2.1 Evaluation
Protocol}\label{fa_15:evaluation-protocol}
...
Backward}\label{fa_15:inv-1-bpb-monotone-backward}
...
\subsection{2.3 Warmup Gate}\label{fa_15:warmup-gate}
...
Architecture}\label{fa_15:railway-write-back-architecture}

\subsection{3.1 Database
Schema}\label{fa_15:database-schema}
...
\subsection{3.3 Gate
Evaluation}\label{fa_15:gate-evaluation}
...
\section{4. Results /
Evidence}\label{fa_15:results-evidence}
...
\section{5. Qed
Assertions}\label{fa_15:qed-assertions}
...
\section{6. Sealed Seeds}\label{fa_15:sealed-seeds}
...
\section{7. Discussion}\label{fa_15:discussion}
...
\section{References}\label{fa_15:references}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\textwidth]{W8-15-kepler-solids-benchmark.png}
  \caption{THE KEPLER / THE BENCH / THE WRITE. Anchor: $\varphi^2+\varphi^{-2}=3$. [AI-generated illustration, style anchor: title-page triptych]}
  \label{fig:wave8-W8-15-kepler-solids-benchmark}
\end{figure}
