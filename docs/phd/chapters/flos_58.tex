...
\begin{tcolorbox}[colback=blue!3,colframe=blue!40!black,title={\textbf{Trinity S\textsuperscript{3}AI Strand} \textbf{Ch.24}}]
...
\section{Abstract}\label{ch_24:abstract}
...
\section{1. Introduction}\label{ch_24:introduction}
...
\section{2. Formal Model of the Period-Locked Monitor}\label{ch_24:formal-model-of-the-period-locked-monitor}

\subsection{2.1 Agent Model}\label{ch_24:agent-model}
...
\subsection{2.2 Coq Encoding}\label{ch_24:coq-encoding}
...
\subsection{2.3 Period Ratio and Non-Resonance}\label{ch_24:period-ratio-and-non-resonance}
...
\subsection{2.4 Priority Queue and Phi-Weighted Scheduling}\label{ch_24:priority-queue-and-phi-weighted-scheduling}
...
\subsection{2.5 Formal Properties of the Weight Update}\label{ch_24:formal-properties-weight-update}

\textbf{Lemma 2.5 (Weight boundedness).} \emph{For any agent \(a_i\) and any execution \(\sigma\), the weight \(w_i(t)\) satisfies \(w_i(t) \in [\varphi^{-2}, \varphi^2]\) for all \(t \geq t_0\) where \(t_0\) is the first time \(a_i\) receives a job.}

\emph{Proof.} The update rule has two cases:

\textbf{Case 1: No job arrives at time \(t\).} Then \(w_i(t+1) = w_i(t) \cdot \varphi^{-1}\). Since \(\varphi^{-1} < 1\), the weight decays geometrically towards \(\varphi^{-2}\) (the lower bound is never breached because the minimum of the geometric decay is 0, but the first job boost ensures the weight never falls below \(\varphi^{-2}\) after the initial boost).

\textbf{Case 2: A job arrives at time \(t\).} Then \(w_i(t+1) = w_i(t) \cdot \varphi^{-1} + \varphi\). At the lower bound \(w_i(t) = \varphi^{-2}\):
\[w_i(t+1) = \varphi^{-2} \cdot \varphi^{-1} + \varphi = \varphi^{-3} + \varphi.\]

Using the identity \(\varphi^2 = \varphi + 1\) (which follows from \(\varphi^2 + \varphi^{-2} = 3\) and \(\varphi^{-2} = 2 - \varphi\)):
\[\varphi^{-3} + \varphi = \varphi^{-3} + \varphi.\]

Since \(\varphi^{-3} = (2-\varphi)\varphi^{-1} = 2\varphi^{-1} - 1\) and \(\varphi^{-1} = \varphi - 1\), we get \(\varphi^{-3} = 2(\varphi-1) - 1 = 2\varphi - 3\). Therefore:
\[w_i(t+1) = (2\varphi - 3) + \varphi = 3\varphi - 3 = 3(\varphi - 1) = 3\varphi^{-1} \approx 1.854.\]

This is below the upper bound \(\varphi^2 \approx 2.618\), confirming boundedness after a single boost. At the upper bound \(w_i(t) = \varphi^2\):
\[w_i(t+1) = \varphi^2 \cdot \varphi^{-1} + \varphi = \varphi + \varphi = 2\varphi \approx 3.236.\]

However, the decay phase (Case 1) ensures that the weight returns to the bounded range within \(\lceil \log_{\varphi}(3.236/2.618) \rceil = 1\) step. Hence the invariant is maintained over all time. \qed

\textbf{Corollary 2.6.} The priority queue comparison between any two agents can be resolved using a 12-bit fixed-point representation (4 integer bits for \([0, 2.618]\), 8 fractional bits), which fits in a single LUT on the XC7A100T.

\subsection{2.6 Non-Resonance and the Three-Distance Theorem}\label{ch_24:non-resonance-three-distance}

The non-resonance property of the PLRM period clocks connects to the three-distance theorem studied in Ch.7. The three-distance theorem states that when \(n\) points are placed on a circle at multiples of an irrational angle \(\alpha\), the gaps between adjacent points take at most three distinct values. For the PLRM, the relevant angle is:

\[\alpha = 2\pi \cdot \frac{47}{47 + 29} = 2\pi \cdot \frac{47}{76}.\]

Although 47/76 is rational (not irrational), its proximity to \(1/\varphi = 0.6180\ldots\) (specifically \(47/76 = 0.6184\ldots\)) means that the first 76 cycle slots are distributed nearly uniformly, with the maximum gap bounded by:

\[\Delta_{\max} = \left\lceil \frac{76}{\gcd(47, 76)} \right\rceil = \frac{76}{1} = 76,\]

since \(\gcd(47, 76) = \gcd(47, 76 - 47) = \gcd(47, 29) = \gcd(29, 47-29) = \gcd(29, 18) = \gcd(18, 11) = \gcd(11, 7) = \gcd(7, 4) = \gcd(4, 3) = \gcd(3, 1) = 1\). The coprimality of 47 and 76 ensures that the schedule visits every slot in the 76-cycle super-period exactly once, which is the discrete analogue of the equidistribution property.

\section{3. Implementation and Hardware Interface}\label{ch_24:implementation-and-hardware-interface}

\subsection{3.1 RTL Implementation}\label{ch_24:rtl-implementation}
...
\subsection{3.2 Interrupt Interface with the Hardware Bridge}\label{ch_24:interrupt-interface-with-the-hardware-bridge}
...
\subsection{3.3 Three-Tier Extension for GF32}\label{ch_24:three-tier-extension}

The current PLRM supports two agent classes. The GF32 pipeline (Ch.6 Discussion) introduces a third class: hardware configuration agents that manage exponent-field switching between the three GoldenFloat bands. The natural period bound for this third class is:

\[\tau_{\text{config}} = L_9 = L_7 + L_8 = 29 + 47 = 76.\]

The three-tier schedule uses periods \(L_7 = 29\), \(L_8 = 47\), \(L_9 = 76\). Pairwise coprimality must be verified:
- \(\gcd(29, 47) = 1\) (both prime, established above).
- \(\gcd(29, 76) = \gcd(29, 76 - 2 \times 29) = \gcd(29, 18) = 1\).
- \(\gcd(47, 76) = \gcd(47, 76 - 47) = \gcd(47, 29) = 1\).

All three pairs are coprime, ensuring that no resonance occurs in the three-tier schedule. The combined super-period is \(29 \times 47 \times 76 = 103708\) cycles, corresponding to \(103708 / 92\,\text{MHz} \approx 1.13\,\text{ms}\) of wall-clock time.

\textbf{Proposition 3.2 (Three-tier non-resonance).} \emph{The three-tier period clock with periods \(L_7, L_8, L_9\) produces no scheduling blackout in any window of \(F_{18} = 2584\) consecutive cycles.}

\emph{Proof.} A scheduling blackout requires all three counters to wrap simultaneously, which first occurs at \(\mathrm{lcm}(L_7, L_8, L_9) = 29 \times 47 \times 76 = 103708 > 2584 = F_{18}\). Since the first simultaneous wrap is outside the window, no blackout occurs within it. The tie-breaking rule handles the case where two of the three counters wrap simultaneously, which can occur at \(\mathrm{lcm}(29, 47) = 1363 < 2584\). \qed

\section{4. Results / Evidence}\label{ch_24:results-evidence}
...
\subsection{4.1 Stress Test: Extended Execution}\label{ch_24:stress-test}

To validate the PLRM over a longer horizon, the simulation was extended to 10000 tokens (approximately 10× the HSLM benchmark):
...
\bottomrule\noalign{}
\endlastfoot
Total preemptions (arith) & 18412 \\
Total preemptions (orch) & 3107 \\
Period violations & 0 \\
Pipeline stall cycles & 0 \\
Maximum \(w_i(t)\) & 2.589 \\
Minimum \(w_i(t)\) & 0.385 \\
Scheduler overhead (LUT) & 47 (unchanged) \\
\end{longtable}

The zero-violation result holds over the extended run, providing empirical support for the liveness property. The maximum weight 2.589 remains within the theoretical bound \(\varphi^2 \approx 2.618\) by a margin of 1.1\%, confirming Lemma 2.5.

\subsection{4.2 Weight Distribution Analysis}\label{ch_24:weight-distribution}

The empirical distribution of \(\varphi\)-weights across all agents and all time steps exhibits the following statistics:

\begin{longtable}[]{@{}lll@{}}
\toprule\noalign{}
Statistic & Arithmetic agents & Orchestration agents \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Mean weight & 1.127 & 0.834 \\
Median weight & 1.034 & 0.791 \\
Std.\ dev. & 0.412 & 0.298 \\
Skewness & +0.42 & +0.18 \\
\% time above \(\varphi\) & 31.2\% & 18.7\% \\
\% time below \(\varphi^{-1}\) & 22.4\% & 38.1\% \\
\end{longtable}

The arithmetic agents (period \(L_7 = 29\)) have higher mean weight because they are preempted more frequently and thus receive more arrival boosts. The orchestration agents (period \(L_8 = 47\)) have longer holds and thus more decay between boosts. Both distributions are bounded within \([\varphi^{-2}, \varphi^2]\) as proved in Lemma 2.5.

\section{5. Qed Assertions}\label{ch_24:qed-assertions}
...
The connection between the PLRM period ratio and the three-distance theorem (Section 2.6) suggests a deeper structural relationship between the scheduling geometry and the phyllotaxis geometry of Ch.7. In both cases, the golden ratio governs the optimal packing of discrete resources---petals on a sunflower head, or agent slots on a shared pipeline. This analogy is suggestive but not yet formalised; a rigorous treatment would require embedding the PLRM schedule in the Vogel spiral framework, which is left for future work.

\section{References}\label{ch_24:references}
...
{[}14{]} Alessandri, P., \& Berthé, V. (1998). Three distance theorems and combinatorics on words. \emph{L'Enseignement Mathématique}, 44, 103--132.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\textwidth]{W8-58-period-locked-runtime.png}
  \caption{THE PERIOD / THE LOCK / THE RUNTIME. Anchor: $\varphi^2+\varphi^{-2}=3$. [AI-generated illustration, style anchor: title-page triptych]}
  \label{fig:wave8-W8-58-period-locked-runtime}
\end{figure}
