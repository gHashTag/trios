...
\section*{Period-Locked Runtime Monitor}
\addcontentsline{toc}{section}{Period-Locked Runtime Monitor}
...
\subsection{2. Formal Model of the Period-Locked Monitor}\label{formal-model-of-the-period-locked-monitor}
...
The PLRM is formalised in \texttt{t27/proofs/canonical/} as a state-transition system over a discrete time domain \(\mathbb{N}\). The safety property is encoded as:
...
\subsubsection{2.3 Period Ratio and Non-Resonance}\label{period-ratio-and-non-resonance}
...
\subsubsection{2.4 Priority Queue and Phi-Weighted Scheduling}\label{priority-queue-and-phi-weighted-scheduling}
...
\subsection{3. Implementation and Hardware Interface}\label{implementation-and-hardware-interface}

\subsubsection{3.1 RTL Implementation}\label{rtl-implementation}
...
\subsubsection{3.2 Interrupt Interface with the Hardware Bridge}\label{interrupt-interface-with-the-hardware-bridge}
...
(The scheduling safety theorem \texttt{plrm\_mutual\_exclusion} (SCH-1) and its supporting lemmas SCH-2 through SCH-5 reside in \texttt{t27/proofs/canonical/}; SCH-3 through SCH-5 carry Admitted status pending Iris integration as detailed in Ch.18.)
...
{[}1{]} \texttt{gHashTag/trios\#418} --- Ch.24 Period-Locked Runtime Monitor scope issue.

{[}2{]} Lucas, E. (1878). Théorie des fonctions numériques simplement périodiques. \emph{American Journal of Mathematics}, 1(2), 184--196.

{[}3{]} Jung, R. et al.~(2018). Iris from the Ground Up: A Modular Foundation for Higher-Order Concurrent Separation Logic. \emph{Journal of Functional Programming}, 28, e20. https://doi.org/10.1017/S0956796818000151

{[}4{]} \texttt{gHashTag/t27/proofs/canonical/} --- SCH-1 through SCH-5 scheduling theorems. Coq canonical archive.
...
{[}9{]} Vogel, H. (1979). A better way to construct the sunflower head. \emph{Mathematical Biosciences}, 44(3--4), 179--189. https://doi.org/10.1016/0025-5564(79)90080-4
