...
\section*{BPB Benchmark + Neon Write}
\addcontentsline{toc}{section}{BPB Benchmark + Neon Write}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,keepaspectratio]{ch15-bpb-benchmark.png}
\caption*{Figure — Ch.15: BPB Benchmark + Neon Write.}
\end{figure}
...
\subsection{2. BPB Protocol and Monotone Backward Invariant (INV-1)}\label{bpb-protocol-and-monotone-backward-invariant-inv-1}

\subsubsection{2.1 Evaluation Protocol}\label{evaluation-protocol}
...
\subsubsection{2.2 INV-1: BPB Monotone Backward}\label{inv-1-bpb-monotone-backward}

\textbf{Invariant INV-1} (\texttt{Trinity.Canonical.Igla.INV1\_BpbMonotoneBackward}, \texttt{t27/proofs/canonical/igla/INV1\_BpbMonotoneBackward.v} {[}7{]}) states:
...
\subsubsection{2.3 Warmup Gate}\label{warmup-gate}
...
\subsection{3. Neon Write-Back Architecture}\label{neon-write-back-architecture}

\subsubsection{3.1 Database Schema}\label{database-schema}

The Neon PostgreSQL instance (project \texttt{golden-sunflowers-bench}, region \texttt{eu-central-1}) stores training telemetry in the following schema:
...
\subsubsection{3.2 Write-Back Protocol}\label{write-back-protocol}
...
\subsubsection{3.3 Gate Evaluation}\label{gate-evaluation}
...
\textbf{Neon write throughput:} 2,347 rows inserted across 5 training runs with 0 write failures and 0 seed-constraint violations.

\subsection{5. Qed Assertions}\label{qed-assertions}

No Coq theorems are directly anchored to this chapter's output files. The relevant obligations --- INV-1 (9 Qed) and INV-7 (victory criterion) --- are tracked in the Golden Ledger under the \texttt{igla/} subdirectory of \texttt{t27/proofs/canonical/}. The champion lr \(= 0.004\) is certified by INV-1.
...
\tightlist
\item
  \textbf{INV-1} (invariant, golden) --- \texttt{https://github.com/gHashTag/t27/blob/feat/canonical-coq-home/proofs/canonical/igla/INV1\_BpbMonotoneBackward.v} --- linked to Ch.10 and Ch.15 --- \(\varphi\)-weight: \(1.0\) --- notes: BPB monotone backward, lr=0.004 (9 Qed).
\end{itemize}
...
{[}7{]} gHashTag/t27, \texttt{proofs/canonical/igla/INV1\_BpbMonotoneBackward.v}. GitHub. https://github.com/gHashTag/t27/blob/feat/canonical-coq-home/proofs/canonical/igla/INV1\_BpbMonotoneBackward.v
