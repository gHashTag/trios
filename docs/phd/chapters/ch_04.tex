...
\caption{Azbuka of the Tridevyatoe — ch\_04: the wheel / the axle / the cart (Trinity S\textsuperscript{3}AI v5.14, da Vinci codex).}
\end{figure}
...
\caption{Azbuka of the Tridevyatoe — 360-Lane Phi-Distance Grid (variant b) (Trinity S\textsuperscript{3}AI v5.19, da Vinci codex).}
\end{figure}
...
\caption{Azbuka of the Tridevyatoe — Benchmarks: Period Cycles (figure b) (Trinity S\textsuperscript{3}AI v5.20, da Vinci codex).}
\end{figure}
...
\caption{Azbuka of the Tridevyatoe — Trinity GF16 ASIC as a Self-Sovereign dePIN Mesh Node: Zero-DSP Inference and On-Chip Routing at 50,mW (figure b) (Trinity S\textsuperscript{3}AI v5.20, da Vinci codex).}
\end{figure}
...
\section{S1. The Spectral Parameter \(\alpha_{\varphi}\)}\label{ch4-s1-alpha-phi}
...
\section{S2. Dimensional Analysis}\label{ch4-s2-dimensional}
...
derivatives.}
\label{tab:ch4-dimensional}
\end{table}

The choice of \(E_{0}\) and \(\nu_{0}\) is set by the FPGA clock
domain (Ch.\,33): \(\nu_{0}=92\,\mathrm{MHz}\), \(E_{0}=h\nu_{0}\).

\section{S3. Comparison with \(\alpha_{\mathrm{QED}}\)}\label{ch4-s3-alpha-qed}
...
\section{S4. Derivation Levels and the Coq Catalogue}\label{ch4-s4-derivation-levels}
...
\section{S5. Runtime Witness Pointers}\label{ch4-s5-runtime}
...
\section{S6. The Gate Derivation in Closed Form}\label{ch4-s6-gate}
