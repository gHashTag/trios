...
\section*{STROBE Sealed Seeds}
\addcontentsline{toc}{section}{STROBE Sealed Seeds}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,keepaspectratio]{ch13-strobe-sealed-seeds.png}
...
\subsection{2. The STROBE Seed Admissibility Criterion}\label{the-strobe-seed-admissibility-criterion}
...
\subsection{\texorpdfstring{3. The Runtime-Mirror Contract and \texttt{igla\_assertions.json}}{3. The Runtime-Mirror Contract and igla\_assertions.json}}\label{the-runtime-mirror-contract-and-igla_assertions.json}
...
\item
  \textbf{Forbidden-threshold guard}: the theorem \texttt{old\_threshold\_kills\_champion} certifies that the old threshold of 2.65 would have pruned at least one champion candidate, justifying the upgrade to 3.5.
\end{enumerate}
...
\tightlist
\item
  \texttt{trinity\_identity} (\texttt{gHashTag/t27/proofs/canonical/igla/INV2\_IglaAshaBound.v}) --- \emph{Status: Qed} --- \(\varphi^2 + (1/\varphi)^2 = 3\); the Trinity anchor identity.
\item
  \texttt{phi\_pos} (\texttt{gHashTag/t27/proofs/canonical/igla/INV2\_IglaAshaBound.v}) --- \emph{Status: Qed} --- \(\varphi > 0\); positivity of the golden ratio.
\item
  \texttt{phi\_gt\_1} (\texttt{gHashTag/t27/proofs/canonical/igla/INV2\_IglaAshaBound.v}) --- \emph{Status: Qed} --- \(\varphi > 1\); the golden ratio exceeds unity.
\item
  \texttt{asha\_champion\_survives} (\texttt{gHashTag/t27/proofs/canonical/igla/INV2\_IglaAshaBound.v}) --- \emph{Status: Qed} --- For all champion candidates \(b\) and threshold \(\tau \geq 3.5\), the ASHA pruner does not eliminate \(b\).
\item
  \texttt{old\_threshold\_kills\_champion} (\texttt{gHashTag/t27/proofs/canonical/igla/INV2\_IglaAshaBound.v}) --- \emph{Status: Qed} --- There exists a champion candidate that the old threshold 2.65 would have pruned; justifies the threshold upgrade.
\item
  \texttt{phi\_inv4\_approx} (\texttt{gHashTag/t27/proofs/canonical/igla/INV2\_IglaAshaBound.v}) --- \emph{Status: Qed} --- \((1/\varphi)^4 < 0.5\); bounds the fourth-power correction to the ASHA threshold.
\end{itemize}
...
\tightlist
\item
  \textbf{INV-2} (invariant, golden) --- \texttt{gHashTag/t27/proofs/canonical/igla/INV2\_IglaAshaBound.v} --- https://github.com/gHashTag/t27/blob/feat/canonical-coq-home/proofs/canonical/igla/INV2\_IglaAshaBound.v --- ASHA threshold \(3.5 = \varphi^2 + \varphi^{-2} + \varphi^{-4}\). Linked: Ch.13, App.E.
\item
  \textbf{SANCTIONED-SEEDS} (config, golden) --- https://github.com/gHashTag/trios/issues/395 --- \(F_{17}=1597\), \(F_{18}=2584\), \(F_{19}=4181\), \(F_{20}=6765\), \(F_{21}=10946\), \(L_7=29\), \(L_8=47\). Linked: Ch.13, App.E.
\end{itemize}
...
{[}3{]} \texttt{gHashTag/t27/proofs/canonical/igla/INV2\_IglaAshaBound.v}. https://github.com/gHashTag/t27/blob/feat/canonical-coq-home/proofs/canonical/igla/INV2\_IglaAshaBound.v

{[}4{]} Zenodo DOI bundle B004 --- Queen Lotus Adaptive Reasoning. https://doi.org/10.5281/zenodo.19227871

{[}5{]} \texttt{gHashTag/trios\#395} --- Sanctioned seed registry. https://github.com/gHashTag/trios/issues/395
...
{[}11{]} \texttt{gHashTag/t27\#569} --- STROBE precondition tracking. https://github.com/gHashTag/t27/issues/569
