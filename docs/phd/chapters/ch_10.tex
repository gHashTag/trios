...
\section*{Coq L1 Range\(\times\)Precision Pareto}
\addcontentsline{toc}{section}{Coq L1 Range\(\times\)Precision Pareto}
...
\subsection{2. GF(16) Range and Precision Formalisation}\label{gf16-range-and-precision-formalisation}
...
\subsection{3. The Pareto Frontier and Conjecture C1}\label{the-pareto-frontier-and-conjecture-c1}
...
\textbf{Theorem 3.2 (INV-4 entropy band).} Formally verified in \texttt{igla/INV4\_NcaEntropyBand.v} (φ-weight 0.618): the NCA lattice with \(81 = 3^4\) cells maintains the entropy band
...
\textbf{Theorem 3.3 (INV-9 EMA decay validity).} Verified in \texttt{igla/INV9\_EmaDecayValid.v} (8 Qed, φ-weight 0.618): the exponential moving average decay
...
\tightlist
\item
  \texttt{coeff\_53\_pos} (\texttt{gHashTag/t27/proofs/canonical/kernel/Phi.v}) --- \emph{Status: Qed} --- Positivity of the 53-bit coefficient used in the rational approximation of \(\phi\).
\item
  \texttt{sqrt5\_sq} (\texttt{gHashTag/t27/proofs/canonical/kernel/Phi.v}) --- \emph{Status: Qed} --- Certified arithmetic: \(\sqrt{5} \cdot \sqrt{5} = 5\).
\item
  \texttt{sqrt5\_pos} (\texttt{gHashTag/t27/proofs/canonical/kernel/Phi.v}) --- \emph{Status: Qed} --- \(0 < \sqrt{5}\).
\item
  \texttt{sqrt4} (\texttt{gHashTag/t27/proofs/canonical/kernel/Phi.v}) --- \emph{Status: Qed} --- \(\sqrt{4} = 2\).
\item
  \texttt{sqrt5\_gt\_2} (\texttt{gHashTag/t27/proofs/canonical/kernel/Phi.v}) --- \emph{Status: Qed} --- \(2 < \sqrt{5}\), prerequisite for \(\phi > 1\).
\item
  \texttt{phi\_pos} (\texttt{gHashTag/t27/proofs/canonical/kernel/Phi.v}) --- \emph{Status: Qed} --- \(0 < \phi = (1+\sqrt{5})/2\).
\end{itemize}
...
\tightlist
\item
  \textbf{INV-1} (invariant) --- \texttt{gHashTag/t27/proofs/canonical/igla/INV1\_BpbMonotoneBackward.v} --- Status: golden --- Links Ch.10, Ch.15. Notes: BPB monotone backward, lr=0.004. φ-weight: 1.0.
\item
  \textbf{INV-1b} (invariant) --- \texttt{gHashTag/t27/proofs/canonical/igla/INV1b\_LrPhiOptimality.v} --- Status: golden --- Links Ch.10. Notes: lr\_phi optimality (5 Qed). φ-weight: 0.618033988768953.
\item
  \textbf{INV-4} (invariant) --- \texttt{gHashTag/t27/proofs/canonical/igla/INV4\_NcaEntropyBand.v} --- Status: golden --- Links Ch.10, Ch.16. Notes: NCA 81=3⁴. φ-weight: 0.618033988768953.
\item
  \textbf{INV-9} (invariant) --- \texttt{gHashTag/t27/proofs/canonical/igla/INV9\_EmaDecayValid.v} --- Status: golden --- Links Ch.10. Notes: EMA decay 8 Qed. φ-weight: 0.618033988768953.
\item
...
{[}3{]} \texttt{gHashTag/t27\#569} --- Canonical NCA entropy band merge. GitHub issue tracker.

{[}4{]} \texttt{gHashTag/t27/proofs/canonical/igla/INV1\_BpbMonotoneBackward.v} --- INV-1 BPB monotone backward.

{[}5{]} \texttt{gHashTag/t27/proofs/canonical/igla/INV1b\_LrPhiOptimality.v} --- INV-1b lr-phi optimality (5 Qed).

{[}6{]} \texttt{gHashTag/t27/proofs/canonical/igla/INV4\_NcaEntropyBand.v} --- INV-4 NCA entropy band (12 Qed). φ-weight 0.618.

{[}7{]} B005 --- Tri Language Formal DSL. Zenodo, DOI: 10.5281/zenodo.19227873.

{[}8{]} \texttt{gHashTag/t27/proofs/canonical/igla/INV9\_EmaDecayValid.v} --- INV-9 EMA decay (8 Qed).
