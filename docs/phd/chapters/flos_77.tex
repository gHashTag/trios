...
\begin{enumerate}
  \item \textbf{Lever \#1 — Platinum LUT PE (Lane~V):}
        ...
        LUT primitive introduced in \cite{platinum_lut_pe_2026}.
  ...
  \item \textbf{Lever \#2 — BitROM Bidirectional Bank (Lane~W):}
        ...
        macro-cells fabricated at 65\,nm per \cite{bitrom_2025}.
...
\citet{platinum_lut_pe_2026} introduce the \emph{Platinum mesh} LUT primitive,
...
prime implicants is $\approx 9.3$ (see the analysis in \cite{platinum_lut_pe_2026}
...
\citet{bitrom_2025} describe a bidirectional ROM macro-cell manufactured
...
\begin{table}[ht]
  ...
  \begin{tabular}{llllll}
    ...
    BIO  & \texttt{0xDF} & \texttt{OP\_LUT\_LOOKUP}   & V &
      Platinum LUT PE \cite{platinum_lut_pe_2026} & \texttt{lut\_no\_star.v::lut\_lookup\_bio} \\
    LANG & \texttt{0xE0} & \texttt{OP\_BITROM\_READ}  & W &
      BitROM bank \cite{bitrom_2025} & \texttt{bitrom\_no\_star.v::bitrom\_read\_lang} \\
    \bottomrule
  \end{tabular}
\end{table}
...
per the ASP-DAC 2026 profiling data in \cite{platinum_lut_pe_2026}.
...
\end{equation}
%
The bidirectional read capability of \cite{bitrom_2025} permits forward
...
\begin{theorem}[Holographic Lever Stack Composition]
  ...
  \end{equation}
  %
  In particular, with $g_v = 1.4$ (Platinum LUT, \cite{platinum_lut_pe_2026}) and
  $g_w = 2.0$ (BitROM, \cite{bitrom_2025}), the projected efficiency is
  %
  \begin{equation}
...
\begin{enumerate}
  \item \textbf{Process node gap}: The BitROM result of \cite{bitrom_2025}
...
%
\begin{itemize}
  \item \cite{platinum_lut_pe_2026} --- Platinum MST LUT PE, ASP-DAC 2026
        (arXiv:2511.21910).
  \item \cite{bitrom_2025} --- BitROM bidirectional bank, 65\,nm
        (arXiv:2509.08542).
...
\begin{itemize}
  ...
  \item The BitROM read access time is $\leq 0.45\,\text{ns}$ at $65\,\text{nm}$
        (confirmed by \cite{bitrom_2025} Table~2, worst-case slow corner).
