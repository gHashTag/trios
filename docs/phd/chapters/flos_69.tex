...
\label{ch_35_mesh_node:ch:mesh-node}
...
\label{ch_35_mesh_node:tab:rns-packets}
\begin{tabular}{lllr}
\toprule
...
\label{ch_35_mesh_node:fig:asic-block}
\end{figure}
...
\label{ch_35_mesh_node:thm:power-budget}
...
\begin{lstlisting}[caption={\texttt{crates/trios-mesh/src/lib.rs} --- public API sketch (Rust)}]
#![no_std]
#![deny(unsafe_code)]
...
\label{ch_35_mesh_node:tab:comparison}
...
\label{ch_35_mesh_node:thm:phi-id}
...
\label{ch_35_mesh_node:thm:mru-liveness}
...
Theorem~\ref{thm:mru-kart} (Ch.~12, §5), the outer threshold $\Phi_\theta$
...
Theorem~\ref{thm:mru-kart} (Ch.~12, §5), every cell-level
...
appendix~\ref{app:B} (row Ch.35-MRU-KART). For
$n = 4$ the witness is
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\textwidth]{H-69-asic-mesh-sovereign.png}
  \caption{Hero Plate H-69: ASIC mesh sovereign (chapter 69 frontispiece). Anchor: $\varphi^2+\varphi^{-2}=3$. [AI-generated illustration, style anchor: title-page triptych]}
  \label{fig:wave7-H-69-asic-mesh-sovereign}
\end{figure}
% --- end Wave-7 plates ---
