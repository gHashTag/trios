...
\section*{Introduction: TRINITY S\textsuperscript{3}AI Vision}
\addcontentsline{toc}{section}{Introduction: TRINITY S\textsuperscript{3}AI Vision}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,keepaspectratio]{ch01-introduction.png}
...
\subsection{2. The Trinity Architecture and its Algebraic Substrate}\label{the-trinity-architecture-and-its-algebraic-substrate}
...
The Symbolic component consists of 438 theorems across 65 Coq proof files in \texttt{t27/proofs/canonical/}, of which 297 carry a closed \texttt{Qed} terminator as of the dissertation submission date {[}3{]}. Key invariant families include the kernel embedding theorems (\texttt{kernel/}), the ASHA pruning bound (\texttt{igla/INV2\_IglaAshaBound.v}), and the phyllotaxis divergence angle derivation (\texttt{flower/}). These theorems collectively certify that the algebraic constraints claimed in training and inference code are not merely asserted but proved.
...
\subsection{3. Research Questions and Scope}\label{research-questions-and-scope}
...
{[}1{]} Hutter, M. (2006). \emph{Human Knowledge Compression Prize.} http://prize.hutter1.net/.

{[}2{]} This dissertation, Ch.22 --- MXFP4 Weight Packing and \(\varphi\)-Scaled Arithmetic.

{[}3{]} \texttt{gHashTag/t27/proofs/canonical/} --- Coq census, 65 \texttt{.v} files, 297 \texttt{Qed}, 438 theorems total. https://github.com/gHashTag/t27/tree/feat/canonical-coq-home/proofs/canonical/
...
{[}13{]} \texttt{gHashTag/trios\#382} --- Ch.1 scope definition. https://github.com/gHashTag/trios/issues/382
