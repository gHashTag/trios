...
\section*{Vogel Phyllotaxis $137.5° = 360°/\varphi^2$}
\addcontentsline{toc}{section}{Vogel Phyllotaxis $137.5° = 360°/\varphi^2$}
...
\subsection{2. From the Trinity Identity to the Golden Angle}\label{from-the-trinity-identity-to-the-golden-angle}
...
\subsection{\texorpdfstring{3. H4 Root System, E8 Lattice, and the \(\varphi\)-Scaled Block Decomposition}{3. H4 Root System, E8 Lattice, and the \textbackslash varphi-Scaled Block Decomposition}}\label{h4-root-system-e8-lattice-and-the-varphi-scaled-block-decomposition}
...
\tightlist
\item
  \texttt{h4\_root\_count} (\texttt{gHashTag/t27/proofs/canonical/kernel/FlowerE8Embedding.v}) --- \emph{Status: Qed} --- \(120 = 248/2\); the H4 half-shell contains exactly half the E8 root count.
\item
  \texttt{h4\_dim\_equals\_twice\_roots} (\texttt{gHashTag/t27/proofs/canonical/kernel/FlowerE8Embedding.v}) --- \emph{Status: Qed} --- \(120 = 2 \times 60\); H4 roots split evenly into positive and negative.
\item
  \texttt{e8\_roots\_decomposition} (\texttt{gHashTag/t27/proofs/canonical/kernel/FlowerE8Embedding.v}) --- \emph{Status: Abort} --- \(E8\_\mathrm{roots} = H4\_\mathrm{block\_1} \cup H4\_\mathrm{block\_2}\); set-theoretic union pending real-closed-field library integration (KER-3).
\item
  \texttt{e8\_flower\_decomposition} (\texttt{gHashTag/t27/proofs/canonical/kernel/FlowerE8Embedding.v}) --- \emph{Status: Qed} --- \(\dim(H4) + \dim(\varphi \cdot H4) = \dim(E8)/2\).
\item
  \texttt{phi\_scaling\_invariant} (\texttt{gHashTag/t27/proofs/canonical/kernel/FlowerE8Embedding.v}) --- \emph{Status: Abort} --- \(\varphi\)-scaling preserves root-set dimension; pending real-closed-field support (KER-3).
\item
  \texttt{trinity\_e8\_h4\_encoding} (\texttt{gHashTag/t27/proofs/canonical/kernel/FlowerE8Embedding.v}) --- \emph{Status: Qed} --- \(\varphi^2 + \varphi^{-2} = 3 \Rightarrow \dim(H4) + \dim(\varphi \cdot H4) = \dim(E8)/2\).
\end{itemize}
...
{[}3{]} \texttt{gHashTag/t27/proofs/canonical/kernel/FlowerE8Embedding.v}. https://github.com/gHashTag/t27/blob/feat/canonical-coq-home/proofs/canonical/kernel/FlowerE8Embedding.v
...
{[}7{]} \texttt{gHashTag/trios\#377} --- Ch.7 scope definition. https://github.com/gHashTag/trios/issues/377
