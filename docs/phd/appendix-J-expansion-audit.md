...
- **§J.7 BLK ↔ chapter / Coq / Zenodo cross-link** — 5-row table mapping each blocker to Reproducibility chapter (`ch_20:abstract`), Coq INV (none — wetware-firmware boundary, deliberately outside Coq scope per Appendix F charter), and the B002 FPGA mirror (`github:gHashTag/trinity-fpga`, linked from Z-02 = `10.5281/zenodo.19227867`). 3/5 verified rows are honestly marked `audit-pending`. (R5-honest audit, PASS-6, 2026-05-12: previously listed DOI `10.5281/zenodo.19227884` resolved to an unrelated Spanish clinical-simulation study and has been removed; the actual JSONL bring-up log lives in the GitHub mirror.)
