...
\caption{Azbuka triptych: Deferred chapter: Lexicon (Trinity S\textsuperscript{3}AI v5.16, da Vinci codex).}
\end{figure}
...
\section*{The words behind the numbers}

Every technical system earns its own dialect, and Trinity S\textsuperscript{3}AI
is no exception. Terms like \emph{GoldenFloat}, \emph{Period-Locked Monitor},
\emph{BPB gate}, and \emph{Flos Aureus} appear throughout the main text with the
confidence of household words. This lexicon collects their definitions in one
place---a single Aleph where every term points back to the chapter that first
gave it meaning, and forward to the theorem or table that grounds it in data.
If a concept felt opaque at 2~a.m. during review, this appendix is the first
place to look; if the definition points to Appendix~B or a Coq\ \texttt{Qed}
block, follow it---the formalism is friendlier than it sounds.
