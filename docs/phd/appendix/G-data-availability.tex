...
\section{Coq Proof Corpus --- Single Source of Truth}
\label{app:G:coq-sot}

The consolidated proof corpus for this monograph lives \textbf{in-tree}, in the
\texttt{gHashTag/trios} monorepo and \texttt{gHashTag/t27} upstream mirror,
distributed across the following roots (deduplicated by SHA-1 in
\filepath{docs/phd/data/phd\_proofs\_inventory\_v2.json}):

\begin{itemize}
  \item \filepath{docs/phd/theorems/} --- 35 \texttt{*.v} files organised by
    sub-bucket (\texttt{trinity/}, \texttt{igla/}, \texttt{sacred/}, \texttt{gravity/},
    \texttt{Kernel/}, \texttt{Theorems/}).  This is the
    \emph{defense-oriented} layout used by Appendix~\ref{app:F}.
  \item \filepath{trinity-clara/proofs/} --- 21 \texttt{*.v} files (16 in
    \filepath{igla/}, 5 at the top level).  This is the
    \emph{runtime-oriented} layout consumed by the IGLA training pipeline
    and the SystemVerilog assertions of TTSKY26a (Lane L-DPC3).
  \item \filepath{crates/trios-chat/proofs/chat/Trinity\_Chat.v} --- the
    258-theorem L-CHAT bucket (Waves 1--30, 220 \texttt{Qed},
    0 \texttt{Admitted}). See Glava~\ref{ch:chat-invariants}.
  \item \filepath{proofs/KAT\_VSA\_Bridge.v} --- the cross-cutting bridge
    between the Kolmogorov--Arnold theorem family and the VSA matmul
    semantics (1 file, 4 theorems).
  \item \filepath{../../t27/proofs/} and \filepath{../../t27/coq/} ---
    upstream Trinity mirror, 11 \texttt{*.v} files contributing
    $+88$~theorems (32 \texttt{Admitted} entries retained for the
    proof-maturation ledger of Glava~\ref{ch:extended-bounds}).
\end{itemize}

\textbf{Consolidated total: 68 unique \texttt{*.v} files,
773 top-level theorems, 93.6\,\% \texttt{Qed}-closed}
(618 \texttt{Qed}, 42 \texttt{Admitted}, see Appendix~\ref{app:F} for the
full inventory). The drop from the canonical-only ratio of 97.1\,\% to the
consolidated ratio of 93.6\,\% is exactly the t27-mirror's
32~\texttt{Admitted} entries, every one of which is explicit in the
proof-maturation ledger of Glava~\ref{ch:extended-bounds}.

\paragraph{Build manifest.}
The canonical \filepath{\_CoqProject} file in the repository root enumerates
73 paths; \texttt{coqc} is invoked per file by the CI workflow
\texttt{coq-verify.yml} on every push.

\paragraph{Runtime mirror.}
The JSON file \filepath{assertions/igla\_assertions.json} is the single
machine-readable mirror of every \texttt{Qed} theorem cited by a runtime
guard. The Rust crate \texttt{trios-phd::defense\_gate} reads this file and
emits \texttt{assert!} statements in production code.  Any drift between
the JSON and the \texttt{*.v} body is a hard build failure
(\texttt{coq-runtime-invariants v1.1} skill).

\paragraph{RAG indexing.}
Every \textsc{Theorem}, \textsc{Lemma}, and \textsc{Corollary} discovered
by the inventory pass is emitted as a separate chunk of
\texttt{ssot.embeddings} (Railway Postgres \texttt{phd-postgres-ssot}) via
\texttt{cargo run -p trios-phd --bin ingest\_rag\_chunks}, making the
formal proofs semantically searchable at defense time.
