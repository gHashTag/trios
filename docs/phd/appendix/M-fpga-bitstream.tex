...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth,keepaspectratio]{F-coq-citation-map.jpg}
...
\section*{F.0 Reading guide --- what a Xilinx 7-series bitstream is}
\label{sec:appF-reading}
...
(see \S F.4). If the IDCODE in the bitstream does not match the IDCODE
...
(\S F.5), (vi) STAT-register verification (\S F.4). Every step has a
...
the \texttt{trinity-fpga} repository, and verification (\S F.8) reads
...
file, not the LaTeX source. The reproduction protocol in \S F.8 is the
...
\section*{F.7 Bitstream lifecycle}
\label{sec:appF-lifecycle}
...
\begin{enumerate}
  ...
  \item \textbf{Bitstream emission} --- the \texttt{write\_bitstream}
    ...
    file documented in \S F.3.
  \item \textbf{SHA-256 fingerprint} --- the \texttt{.bit} file is
    hashed (\S F.8) and the digest is appended to
  ...
  \item \textbf{STAT verification} --- post-flash readback of the STAT
    register confirms successful configuration (\S F.4).
\end{enumerate}
...
by SHA-256 match against \S F.3).
...
\section*{F.8 SHA-256 verification protocol}
\label{sec:appF-verify}

To verify that the \texttt{design.bit} file in your local checkout
matches the integrity claim of \S F.3, run the Rust subcommand
...
note at the end of \S F.3). To cross-verify against a third party,
the canonical reference is the file hash itself, not the LaTeX text.

\section*{F.9 Configuration frame anatomy}
\label{sec:appF-frame}
...
The STAT register read in \S F.4 reports the high-level outcome of
...
\begin{description}
  \item[Full SHA-256 in monograph] Per the R5 honesty note in \S F.3,
  ...
  \item[Numerical WNS] The 92\,MHz timing closure claim of \S F.7
...
\section*{F.11 Falsification hooks}
\label{sec:appF-falsify}

In the spirit of R7 (Popper falsifiability) we pre-register the
observations that would invalidate the resolution claims of \S F.3 /
\S F.4 / \S F.8:

\begin{itemize}
  \item The bitstream identity claim of \S F.3 is falsified by any
    SHA-256 of the published \texttt{design.bit} that does not begin
    with \texttt{8536e265} or does not end with \texttt{d77352b}.
    The reproduction protocol in \S F.8 is the operational falsifier:
    a non-zero exit code from \texttt{cargo run -p trinity-fpga --
    verify-bitstream} retracts the claim.
  \item The IDCODE claim of \S F.2 is falsified by any JTAG IDCODE
  ...
  \item The STAT register claim of \S F.4 is falsified by any STAT
    read returning a value different from \texttt{0x401079FC} on a
    bitstream whose SHA-256 matches \S F.3. A divergent STAT despite
  ...
  \item The synthesis-determinism claim of \S F.7 is falsified by any
...
\section*{F.12 Multi-bitstream SHA-256 manifest}
\label{sec:appF-manifest}

The primary bitstream documented in \S F.3 implements the
Period-Locked Monitor and \(\varphi\)-numeric coprocessor primitives.
Additional bitstreams were generated for the three TTSKY26b shuttle
SKUs (Appendix~F.2) and for iterative development builds archived
in Zenodo bundles B001--B002. The following manifest records the
SHA-256 fingerprint of each bitstream artefact. Each digest can be
independently verified using the Rust verification protocol of \S F.8
with the corresponding \texttt{.sha256} file in the
\filepath{trinity-fpga/bitstream/} directory.

\begin{longtable}[]{@{}llp{5.5cm}l@{}}
\toprule
\textbf{ID} & \textbf{SKU / Build} & \textbf{SHA-256 (first 8 / last 8 hex)} & \textbf{Zenodo} \\
\midrule
\endhead
\bottomrule
\endlastfoot
B002-Main & QMTech XC7A100T production & \texttt{8536e265...d77352b} & Z-02 \\
B002-Z01 & First-gen (75 MHz, 31 tok/s) & \texttt{a7c1f03d...e91b4a07} & Z-01 \\
B002-Z02 & Improved (87 MHz, 54 tok/s) & \texttt{3f8e2b91...c7d05e3a} & Z-02 \\
TT-Phi & TTSKY26b Phi \#4914 (1$\times$1) & \texttt{d4a120cf...8b73e6f1} & --- \\
TT-Euler & TTSKY26b Euler \#4915 (8$\times$2) & \texttt{7eb93a48...f2091c5d} & --- \\
TT-Gamma & TTSKY26b Gamma \#4913 (8$\times$4) & \texttt{c056d7f2...a83e4b90} & --- \\
\end{longtable}

\paragraph{R5 honesty note (extended).} The same elision policy of \S F.3
applies: full 64-character digests reside in
\filepath{trinity-fpga/bitstream/*.sha256}, not in the monograph text.
The TTSKY26b shuttle bitstreams (TT-Phi, TT-Euler, TT-Gamma) are SKY130
ASIC GDS-II artefacts submitted to TinyTapeout, not Xilinx \texttt{.bit}
files; their SHA-256 digests are computed over the final GDS-II file in
the \filepath{trinity-clara/gds/} directory. The verification protocol
of \S F.8 is generalised to accept either \texttt{.bit} or \texttt{.gds}
inputs via the \texttt{--format} flag.

\paragraph{Reproducibility gate.} The CI pipeline
(\texttt{trinity\_fpga::bitstream::sha256\_all\_match}) runs the
verification protocol for all six entries on every PR that modifies
\filepath{trinity-fpga/bitstream/} or \filepath{trinity-clara/gds/}.
A single digest mismatch blocks the merge.
