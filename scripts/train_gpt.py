"""
trios Parameter Golf Submission — train_gpt.py
Target: BPB < 1.15 on FineWeb validation (tokenizer-agnostic)
Deadline: Apr 30, 2026

Techniques stack (based on leaderboard analysis + trios GOLF research):
- SP4096 vocabulary (sentencepiece, 4096 merge rules)
- 11-layer transformer, dim=512, 8 heads, 4 KV heads (GQA)
- 3x MLP with ReLU^2 activation
- RoPE positional encoding
- QK-Norm + QK-Gain (init=5.25)
- BigramHash(3072) + SmearGate
- Muon optimizer for matrices, Adam for embeddings/scalars
- Depth recurrence: loop layers 4-5 twice (effective 13 layers)
- Parallel attention/MLP residual lanes
- Sliding window eval (stride=64)
- EMA (exponential moving average, decay=0.998)
- Int8 quantization + zlib compression (~15.7MB artifact)
...
class Hyperparameters:
    ...
    bigram_dim = int(os.environ.get("BIGRAM_DIM", 576))
    use_smear_gate = bool(int(os.environ.get("USE_SMEAR_GATE", "1")))

    depth_recurrence_start = int(os.environ.get("DEPTH_RECURRENCE_START", 4))
    depth_recurrence_end = int(os.environ.get("DEPTH_RECURRENCE_END", 6))
    depth_recurrence_loops = int(os.environ.get("DEPTH_RECURRENCE_LOOPS", 2))

    use_parallel_residuals = bool(int(os.environ.get("USE_PARALLEL_RESIDUALS", "1")))
...
class BigramHashEmbedding(nn.Module):
    def __init__(self, vocab_size: int, bigram_vocab_size: int, dim: int):
        super().__init__()
        self.vocab_size = vocab_size
        self.bigram_vocab_size = bigram_vocab_size
        self.embed = nn.Embedding(bigram_vocab_size, dim)
        self.proj = CastedLinear(dim, dim, bias=False) if dim != dim else None
    ...
    def bigram_hash(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        mod_val = self.bigram_vocab_size - 1
        batch, seq = t.shape
...
class GPT(nn.Module):
    def __init__(
        ...
        bigram_dim: int,
        use_smear_gate: bool,
        use_parallel_residuals: bool,
        depth_recurrence_start: int,
        depth_recurrence_end: int,
    ):
        super().__init__()
        ...
        self.bigram_emb = BigramHashEmbedding(vocab_size, bigram_vocab_size, bigram_dim)
        self.smear_gate = SmearGateModule(model_dim) if use_smear_gate else None
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            if self.depth_recurrence_start <= layer_idx < self.depth_recurrence_end:
...
def main() -> None:
    ...
    log0(f"trios Parameter Golf Submission — GOLF stack v1.0", console=False)
    ...
    base_model = GPT(
        ...
        use_parallel_residuals=args.use_parallel_residuals,
    ...
    for name, param in base_model.bigram_emb.named_parameters():
        if param.ndim == 2:
            bigram_matrix_params.append(param)
        else:
            bigram_scalar_params.append(param)
    ...
    ema = EMA(base_model, args.ema_decay) if args.use_ema and master_process else None

    n_params = sum(p.numel() for p in base_model.parameters())
    log0(f"model_params:{n_params}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(f"techniques: BigramHash({args.bigram_vocab_size}) + SmearGate + DepthRecur({args.depth_recurrence_start}-{args.depth_recurrence_end}) + ParallelResid + EMA({args.ema_decay}) + QK-Gain({args.qk_gain_init}) + Muon")
    log0(
