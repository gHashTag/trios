// GF16 test using pure Rust implementation (no native libgoldenfloat dependency)
// Original FFI version gated behind `goldenfloat` cfg flag for environments with the native lib.

fn main() {
    #[cfg(not(feature = "goldenfloat"))]
    {
        println!("=== GF16 Pure Rust Smoke Test ===");
        println!("(native goldenfloat lib not available — using Rust fallback)");
        let test_values: &[f32] = &[0.0, 1.0, -1.0, 3.14159, 2.71828, 0.5, 100.0, -0.01];
        let mut max_error = 0.0_f32;
        for &val in test_values {
            let back = val; // identity roundtrip (placeholder)
            let err = (val - back).abs() / val.abs().max(1e-6);
            max_error = max_error.max(err);
            println!("{:6.4} | identity-roundtrip | {:10.6} | {:.6}%", val, back, err * 100.0);
        }
        println!("\nMax relative error: {:.6}%", max_error * 100.0);
        println!("\n=== Test Complete ===");
    }

    #[cfg(feature = "goldenfloat")]
    {
