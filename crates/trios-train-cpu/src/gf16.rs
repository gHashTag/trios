...
impl GF16 {
    ...
    const EXP_BIAS: i32 = 15;  // Adjusted bias to represent 1.0, 2.0, etc.
    ...
    pub fn from_f32(val: f32) -> Self {
        ...
        let f32_exp = ((bits >> 23) & 0xFF) as i32 - 127;  // f32 bias is 127
        let f32_mant = bits & 0x007FFFFF;  // 23 bits

        // Zero and subnormals don't exist in GF16 - clamp to zero
        if f32_exp <= -15 {
            return GF16((f32_sign as u16) << 15);
        }

        // GF16 exponent: bias 15, 6 bits (values 1-62 for normal, 0=zero, 63=inf/NaN)
        let mut exp = f32_exp + Self::EXP_BIAS;

        // Clamp exponent to normal range (1-62)
        if exp >= 63 {
            exp = 62;  // Clamp to max normal
        } else if exp <= 0 {
            return GF16((f32_sign as u16) << 15);
        }

        // Mantissa: take top 9 bits from f32's 23-bit mantissa
    ...
    pub fn to_f32(self) -> f32 {
        ...
        // Infinity or NaN
        if exp == 63 {  // All 6 exponent bits set
            if mantissa == 0 {
...
mod tests {
    ...
    #[test]
    fn test_roundtrip() {
        let values = [0.1, 0.5, 1.0, 2.0, 10.0, 100.0, 0.001, 0.0001];
        for &v in &values {
            let gf = GF16::from_f32(v);
            let back = gf.to_f32();
            let rel_error = (back - v).abs() / v.abs().max(1e-10);
            assert!(rel_error < 0.02, "Roundtrip failed for {}: {} -> {} -> {}, error={}", v, v, back, rel_error);
        }
    }
    ...
    fn test_phi_distance() {
        ...
        assert!((pd - 0.0486).abs() < 0.001, "φ-distance = {}, expected ~0.049", pd);
    }
    ...
    fn test_gf16_vec() {
        ...
        assert_eq!(vec.len(), 3);
        let back: Vec<f32> = vec.to_f32_vec();
...
impl QuantizationMetrics {
    pub fn compute(original: &[f32], quantized: &[f32]) -> Self {
        ...
        for (i, &orig) in original.iter().enumerate() {
            ...
            let abs_orig = orig.abs().max(1e-10);
            let error_pct = ((quant - orig).abs() / abs_orig * 100.0) as f64;
        ...
        QuantizationMetrics {
            ...
            avg_error_pct: sum_error / n as f64,
            mse: sum_sq_error / n as f64,
            mae: sum_abs_error / n as f64,
