...
//! after W33 = 62 (W32+2).
//! Wave-34 additions: 50 ephemeral_mailbox_unlinkability (R-CHAT-3 /
//! L-CHAT-4-emu) + 50 blind_signature_sender_token (R-CHAT-10 /
//! L-CHAT-7-bsst) → 3300/3300 expected; cumulative threshold-lane count
...
//! Wave-38 additions: 50 welcome_init_secret_kdf_label_confusion
//! (R-CHAT-2 / L-CHAT-2-wisklc) + 50 psk_secret_extraction_chain_order_mismatch
//! (R-CHAT-5 / L-CHAT-5-pscom) → 3700/3700 expected; cumulative
//! threshold-lane count after W38 = 72 (W37+2). Lanes implement
//! RFC 9420 §8.4 init_secret KDF label binding + RFC 9420
//! §15.1.1 PSK secret deterministic chain order.
//!
//! Wave-37 additions: 50 key_package_lifetime_grace_window_expiry
//! (R-CHAT-1 / L-CHAT-1-kplgw) + 50 external_commit_resumption_psk_misbinding
//! (R-CHAT-3 / L-CHAT-3-ecrpm) → 3600/3600 expected; cumulative
//! threshold-lane count after W37 = 70 (W36+2). Lanes implement
//! RFC 9420 §10.1 KeyPackage lifetime grace ceiling + RFC 9420
//! §11.2.1+§15.1 ExternalCommit ResumptionPSK binding.
//!
...
fn main() {
    ...
    for (cat, min) in [
        ...
        ("key_package_lifetime_grace_window_expiry", 0.95_f64),
        ("external_commit_resumption_psk_misbinding", 0.95_f64),
        // Wave-38 lanes (RFC 9420 §8.4 init_secret KDF label + §15.1.1 PSK chain order)
        ("welcome_init_secret_kdf_label_confusion", 0.95_f64),
        ("psk_secret_extraction_chain_order_mismatch", 0.95_f64),
    ] {
