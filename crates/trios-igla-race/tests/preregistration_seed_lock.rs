...
/// Locate the workspace root regardless of where Cargo invokes the test from.
fn workspace_root() -> PathBuf {
    // `CARGO_MANIFEST_DIR` is `<root>/crates/trios-igla-race`, so stepping
    // up two directories lands on the workspace root.
...
fn ledger_path() -> PathBuf {
    workspace_root().join("assertions/seed_results.jsonl")
}

fn gate_2_done_marker() -> PathBuf {
    workspace_root().join("assertions/.gate2_done")
}
...
#[test]
fn marker_path_is_under_workspace() {
    // Sanity: the path resolution helpers point inside the workspace.
    ...
    assert!(
        ...
        "workspace_root() resolved to {} which has no Cargo.toml",
        root.display(),
    );
