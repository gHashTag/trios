...
async fn run_worker(
    ...
    _machine_id: &str,
    worker_id: u64,
    _best_bpb: Arc<RwLock<f64>>,
) -> Result<f64> {
    let _db = NeonDb::connect(_neon_url).await?;
    ...
    loop {
        ...
        let _config_value = serde_json::to_value(&config)?;

        let _trial_id = _db.register_trial(
            Uuid::new_v4(),
            _machine_id,
            worker_id as i32,
            serde_json::to_string(&_config_value)?.as_str(),
        ).await?;
        ...
        for rung in AshaRung::all() {
            ...
            _db.client().execute(&query, &[&(step as i32), &(bpb as f64), _trial_id]).await?;
        ...
        let mut best = _best_bpb.write().unwrap();
...
async fn show_best(_neon_url: &str) -> Result<()> {
    let _db = NeonDb::connect(_neon_url).await?;
    let row = _db.client().query_one(
