...
use crate::lessons::{generate_lesson, Outcome, RungData, store_lesson, TrialConfig, LessonType};
...
pub async fn record_checkpoint(
    ...
    let column = match rung {
        AshaRung::Rung1000 => "rung_1000",
        AshaRung::Rung3000 => "rung_3000",
        AshaRung::Rung9000 => "rung_9000",
        AshaRung::Rung27000 => "rung_27000",
    };
    
    let step_col = format!("{}_step", column);
    let bpb_col = format!("{}_bpb", column);
    
    let query = format!(
        r#"
        UPDATE igla_race_trials 
        SET {} = $1, {} = $2, final_step = $1, final_bpb = $2
        WHERE trial_id = $3
        "#,
        step_col, bpb_col
    );
...
pub async fn should_prune(
    ...
    let all_trials: Vec<(f64,)> = sqlx::query_as(
        "SELECT COALESCE(rung_1000_bpb, 999) FROM igla_race_trials WHERE status IN ('running', 'completed') AND rung_1000_bpb IS NOT NULL"
    )
    .fetch_all(pool)
    .await?;
    ...
    let mut sorted: Vec<f64> = all_trials.into_iter().map(|(b,)| b).collect();
...
pub async fn mark_completed(
    ...
    if final_bpb < 1.5 {
        ...
        store_lesson(
            pool,
            trial_id,
            Outcome::Pruned, // Use any outcome for storage
            final_step,
            final_bpb,
            &lesson,
            LessonType::Winner,
...
pub async fn process_rung(
    ...
    // Record checkpoint
    record_checkpoint(pool, trial_id, rung, rung.step(), bpb).await?;
    
    // Check if target reached
    if bpb <= asha_config.target_bpb {
        mark_completed(pool, trial_id, rung.step(), bpb).await?;
        return Ok(false); // Stop trial
    }
    
    // Check if should prune
    if should_prune(pool, trial_id, bpb, asha_config).await? {
        handle_pruning(pool, trial_id, rung, bpb, trial_config).await?;
