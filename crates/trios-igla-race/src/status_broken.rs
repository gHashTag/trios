...
pub async fn print_leaderboard(client: &Client) -> Result<()> {
    ...
    for row in rows {
        let _trial_id: String = row.try_get(0)?;
        let machine_id: String = row.try_get(1)?;
        let final_bpb: String = row.try_get(2)?;
        let final_step: String = row.try_get(3)?;
        let lesson: String = row.try_get(4)?;
        let bpb_rank: i64 = row.try_get(5)?;
...
pub async fn print_best(client: &Client) -> Result<()> {
    ...
    if let Some(row) = row_opt {
        let _trial_id: String = row.try_get(0)?;
        let machine_id: String = row.try_get(1)?;
        let config_json: String = row.try_get(2)?;
        let final_bpb: String = row.try_get(3)?;
        let final_step: String = row.try_get(4)?;
