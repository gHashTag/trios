...
pub async fn dispatch(name: &str, input: Value) -> Result<Value> {
    ...
    if let Some(r) = fs::dispatch(name, &input).await {
        return r;
    }

    // Knowledge Graph tools (no repo_path needed)
    if let Some(r) = trios_kg::dispatch(name, &input).await {
        return r;
    }
    ...
    let repo_path =
        validate_repo_path(raw_repo_path, &allowed_roots()).map_err(|e| anyhow::anyhow!(e))?;
    let repo = repo_path.as_path();
    let git = Git2Orchestrator;

    // Git basic tools
    if let Some(r) = git::dispatch(name, &input, repo, &git).await {
        return r;
    }

    // Git extended tools
    if let Some(r) = git_extended::dispatch(name, &input, repo, &git).await {
        return r;
    }

    // GitButler tools
    if let Some(r) = gitbutler::dispatch(name, &input, repo, &git).await {
        return r;
    }

    // Golden Float tools
    if let Some(r) = golden_float::dispatch(name, &input).await {
        return r;
    }
