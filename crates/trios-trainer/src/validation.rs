//! Validation utilities for IGLA training
//!
//! BPB calculation and champion reproduction validation.
...
///
/// ```rust
/// use trios_trainer::validation::calculate_bpb;
///
...
pub fn calculate_bpb(nll: f32, num_tokens: usize) -> f32 {
    ...
    (log2_perplexity / 8.0_f32.ln()) / 256.0_f32.ln()
}
...
///
/// ```rust
/// use trios_trainer::validation::{calculate_bpb, is_within_champion_tolerance};
///
...
mod tests {
    ...
    fn test_champion_tolerance(\) {
        ...
        assert!(is_within_champion_tolerance(2.2393)); // true
        assert!(is_within_champion_tolerance(2.2293)); // true (min)
        assert!(is_within_champion_tolerance(2.2493)); // true (max)
    ...
    fn test_champion_tolerance_invalid_low() {
        assert!(!is_within_champion_tolerance(2.22)); // false (way below)
    }

    #[test]
    fn test_champion_tolerance_invalid_high() {
        assert!(!is_within_champion_tolerance(2.25)); // false (way above)
    }
}
