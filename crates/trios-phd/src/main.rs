...
fn fix_common_latex(phd_root: &Path) -> Result<()> {
    ...
    // Build a lookup of (numeric prefix 'NN' or letter 'L') -> canonical filename.
    let canonical_index = build_canonical_image_index(&illus, &illus_v516);
    for sub in ["chapters", "appendix"] {
        ...
        for entry in std::fs::read_dir(&dir)? {
            ...
            // 0. Strip standalone-document scaffolding (`\documentclass`,
            //    `\begin{document}`, `\end{document}`) that occasionally
            //    leaks into chapter files autogenerated from Neon SSOT.
            //    An `\end{document}` inside a chapter file silently aborts
            //    tectonic's processing of every subsequent chapter, which
            //    is exactly how 70+ figures went missing from the v71 PDF.
            fixed = strip_document_scaffolding(&fixed);
            // 6. Rewrite legacy `chNN-anything.png` and `app-X-anything.jpg`
            //    \includegraphics references to the canonical NN-slug.png /
            //    LETTER-slug.jpg filename that actually exists on disk.
            let (next, n_remapped) =
                remap_legacy_image_filenames(&fixed, &canonical_index, fname);
            if n_remapped > 0 {
                fixed = next;
                remapped += n_remapped;
            }
    ...
    eprintln!(
        "fix-common-latex: rewrote {} file(s), inserted {} hero figure(s), remapped {} legacy image ref(s)",
        total, hero_inserted, remapped
    );
    Ok(())
}

/// Strip standalone-document scaffolding (`\documentclass`,
/// `\begin{document}`, `\end{document}`) from chapter/appendix files
/// that are meant to be `\include{}`d, never compiled stand-alone.
///
/// Rationale: an `\end{document}` that leaks into a `\include{}`d file
/// SILENTLY aborts tectonic's processing of every subsequent file.
/// This is the root cause of the missing-70-figures bug in v71.
/// Idempotent.
fn strip_document_scaffolding(src: &str) -> String {
    ...
    // Preserve trailing newline behavior: if original had no trailing
    // newline, drop the one we just added.
    if !src.ends_with('\n') && out.ends_with('\n') {
        out.pop();
    }
    out
...
/// Index of canonical image filenames keyed by numeric prefix (`00`..`33`)
/// or single uppercase letter (`A`..`L`). Each value is the basename with
/// extension, e.g. `"12-flower-of-life.png"` or `"E-lexicon.jpg"`.
fn build_canonical_image_index(
    illus: &Path,
    illus_v516: &Path,
) -> std::collections::HashMap<String, String> {
    let mut map = std::collections::HashMap::new();
    for dir in [illus, illus_v516] {
        let Ok(rd) = std::fs::read_dir(dir) else { continue };
        for entry in rd.flatten() {
            let Some(name) = entry.file_name().to_str().map(|s| s.to_string()) else { continue };
            let Some(stem) = name.rsplitn(2, '.').nth(1) else { continue };
            let Some(prefix) = stem.split('-').next() else { continue };
            // numeric NN or single uppercase letter
            let valid_num = prefix.len() == 2 && prefix.chars().all(|c| c.is_ascii_digit());
            let valid_let = prefix.len() == 1 && prefix.chars().all(|c| c.is_ascii_uppercase());
            if !valid_num && !valid_let { continue; }
            // Prefer PNG (illus dir) over JPG (v516) by inserting only if
            // not already present.
            map.entry(prefix.to_string()).or_insert(name);
        }
    }
    map
}

/// Rewrite legacy `\includegraphics{chXX-...png}` or `{app-X-...jpg}`
/// references to the canonical `XX-slug.png` / `X-slug.jpg` filename
/// found in `canonical_index`.
///
/// Returns `(new_source, number_of_replacements)`.
///
/// Special cases (preserve domain-specific mapping, not a pure prefix lookup):
///   - `ch_00.tex` claims `ch29-sacred-formula-v.png` but is actually the
///     Standard-Model chapter; remap to `20-standard-model.png`.
///   - `ch_35*.tex` (mesh node) maps to `34-depin-mesh.jpg`.
fn remap_legacy_image_filenames(
    src: &str,
    canonical_index: &std::collections::HashMap<String, String>,
    file_basename: &str,
) -> (String, usize) {
    let mut out = String::with_capacity(src.len());
    ...
    while let Some(rel) = src[cursor..].find(needle) {
        let abs = cursor + rel;
        // locate the `{` that opens the path argument.
        ...
        let open = j;
        let Some(close) = match_balanced_brace(src, open) else {
            out.push_str(&src[cursor..]);
            return (out, count);
        };
        let path = &src[open + 1..close - 1];
        let replacement = remap_one_image_path(path, canonical_index, file_basename);
...
fn remap_one_image_path(
    path: &str,
    canonical_index: &std::collections::HashMap<String, String>,
    file_basename: &str,
) -> Option<String> {
    // Strip a trailing extension if present (e.g. `.png`); we will pick the
    // extension that the canonical asset actually has on disk.
    let stem = path.rsplitn(2, '.').nth(1).unwrap_or(path);
    let already_canonical = canonical_index
        .values()
        .any(|v| v.rsplitn(2, '.').nth(1).unwrap_or(v) == stem);
    if already_canonical {
        return None; // idempotent: do not touch already-canonical refs
    }
    // Special-case overrides (domain knowledge — Neon SSOT autogen drift).
    if file_basename == "ch_00.tex" {
        if let Some(v) = canonical_index.get("20") {
            if v != path { return Some(v.clone()); }
        }
    }
    if file_basename.starts_with("ch_35") {
        if let Some(v) = canonical_index.get("34") {
            if v != path { return Some(v.clone()); }
        }
    }
    // Pattern 1: `chXX-anything` -> prefix "XX"
    if stem.starts_with("ch") && stem.len() >= 4 {
        let after_ch = &stem[2..];
        let digits: String = after_ch.chars().take_while(|c| c.is_ascii_digit()).collect();
        if digits.len() == 2 {
            if let Some(v) = canonical_index.get(&digits) {
                if v.rsplitn(2, '.').nth(1).unwrap_or(v) != stem {
                    return Some(v.clone());
                }
            }
        }
        // Pattern 1b: `chN-anything` (single digit) -> zero-pad
        let digit1: String = after_ch.chars().take_while(|c| c.is_ascii_digit()).collect();
        if digit1.len() == 1 {
            let padded = format!("0{}", digit1);
            if let Some(v) = canonical_index.get(&padded) {
                return Some(v.clone());
            }
        }
    }
    // Pattern 2: `app-X-anything` or `app_X_anything` -> letter "X"
    let lower = stem.to_ascii_lowercase();
    if lower.starts_with("app-") || lower.starts_with("app_") {
        if let Some(letter_ch) = stem.chars().nth(4) {
            let letter = letter_ch.to_ascii_uppercase().to_string();
            if let Some(v) = canonical_index.get(&letter) {
                return Some(v.clone());
            }
        }
    }
...
mod tests {
    ...
    // -------------------------------------------------------------------
    // Legacy image-filename remap tests (gate for phd-pdf-images-gate).
    // -------------------------------------------------------------------

    fn canonical_index_fixture() -> std::collections::HashMap<String, String> {
        let mut m = std::collections::HashMap::new();
        // Numeric prefixes (PNG preferred).
        for (n, slug) in [
            ("00", "00-monad.png"),
            ("01", "01-golden-egg.png"),
            ("12", "12-flower-of-life.png"),
            ("20", "20-standard-model.png"),
            ("29", "29-sacred-formula.png"),
            ("33", "33-epilogue.png"),
            ("34", "34-depin-mesh.jpg"),
        ] {
            m.insert(n.to_string(), slug.to_string());
        }
        // Letter prefixes (appendix JPGs).
        for (l, slug) in [
            ("A", "A-catalogue.jpg"),
            ("E", "E-lexicon.jpg"),
            ("F", "F-coq-map.jpg"),
            ("L", "L-pollen-channel.jpg"),
        ] {
            m.insert(l.to_string(), slug.to_string());
        }
        m
    }

    #[test]
    fn test_remap_chNN_to_canonical_png() {
        let idx = canonical_index_fixture();
        let src = "\\includegraphics[width=0.8\\linewidth]{ch12-hardware-bridge.png}";
        let (out, n) = remap_legacy_image_filenames(src, &idx, "chapters/ch_12.tex");
        assert_eq!(n, 1);
        assert!(out.contains("{12-flower-of-life.png}"), "got: {}", out);
        assert!(!out.contains("ch12-hardware-bridge"));
    }

    #[test]
    fn test_remap_ch_00_special_case_to_standard_model() {
        let idx = canonical_index_fixture();
        // ch_00.tex points at sacred-formula but IS the Standard Model chapter.
        let src = "\\includegraphics{ch29-sacred-formula-v.png}";
        let (out, n) = remap_legacy_image_filenames(src, &idx, "ch_00.tex");
        assert_eq!(n, 1);
        assert!(out.contains("{20-standard-model.png}"), "got: {}", out);
    }

    #[test]
    fn test_remap_ch_35_to_depin_mesh() {
        let idx = canonical_index_fixture();
        let src = "\\includegraphics{ch35-mesh-rev2.png}";
        let (out, n) = remap_legacy_image_filenames(src, &idx, "ch_35_mesh_node.tex");
        assert_eq!(n, 1);
        assert!(out.contains("{34-depin-mesh.jpg}"), "got: {}", out);
    }

    #[test]
    fn test_remap_appendix_app_letter_to_canonical() {
        let idx = canonical_index_fixture();
        let src = "\\includegraphics{app-E-glossary.jpg}";
        let (out, n) = remap_legacy_image_filenames(src, &idx, "appendix/E-lexicon.tex");
        assert_eq!(n, 1);
        assert!(out.contains("{E-lexicon.jpg}"), "got: {}", out);
    }

    #[test]
    fn test_remap_idempotent_on_canonical_ref() {
        let idx = canonical_index_fixture();
        let src = "\\includegraphics{12-flower-of-life.png}";
        let (out, n) = remap_legacy_image_filenames(src, &idx, "chapters/12-flower-of-life.tex");
        assert_eq!(n, 0, "already-canonical refs must NOT be rewritten");
        assert_eq!(out, src);
    }

    #[test]
    fn test_remap_leaves_unknown_path_untouched() {
        let idx = canonical_index_fixture();
        let src = "\\includegraphics{cover_chalkboard.png}";
        let (out, n) = remap_legacy_image_filenames(src, &idx, "frontmatter/cover.tex");
        assert_eq!(n, 0);
        assert_eq!(out, src);
    }

    #[test]
    fn test_remap_multiple_in_one_file() {
        let idx = canonical_index_fixture();
        let src = concat!(
            "\\includegraphics{ch01-egg.png}\n",
            "text text \\includegraphics[width=2cm]{ch33-coda.png}\n",
            "\\includegraphics{12-flower-of-life.png}\n",
        );
        let (out, n) = remap_legacy_image_filenames(src, &idx, "chapters/ch_01.tex");
        assert_eq!(n, 2, "two legacy refs should be remapped, one canonical left alone");
        assert!(out.contains("{01-golden-egg.png}"));
        assert!(out.contains("{33-epilogue.png}"));
        assert!(out.contains("{12-flower-of-life.png}"));
    }

    #[test]
    fn test_build_canonical_image_index_handles_missing_dirs() {
        let d = tempfile::tempdir().unwrap();
        let missing_a = d.path().join("does-not-exist-a");
        let missing_b = d.path().join("does-not-exist-b");
        let idx = build_canonical_image_index(&missing_a, &missing_b);
        assert!(idx.is_empty());
    }

    #[test]
    fn test_strip_document_scaffolding_removes_end_document() {
        let src = "\\chapter{Sacred Ratios}\nBody.\n\\end{document}\n";
        let out = strip_document_scaffolding(src);
        assert!(!out.contains("\\end{document}"));
        assert!(out.contains("\\chapter{Sacred Ratios}"));
        assert!(out.contains("Body."));
    }

    #[test]
    fn test_strip_document_scaffolding_removes_documentclass_and_begin() {
        let src = "\\documentclass{book}\n\\begin{document}\n\\chapter{X}\n\\end{document}\n";
        let out = strip_document_scaffolding(src);
        assert!(!out.contains("\\documentclass"));
        assert!(!out.contains("\\begin{document}"));
        assert!(!out.contains("\\end{document}"));
        assert!(out.contains("\\chapter{X}"));
    }

    #[test]
    fn test_strip_document_scaffolding_idempotent() {
        let src = "\\chapter{Y}\nText\n";
        let out1 = strip_document_scaffolding(src);
        let out2 = strip_document_scaffolding(&out1);
        assert_eq!(out1, out2);
        assert_eq!(out1, src);
    }

    #[test]
    fn test_strip_document_scaffolding_leaves_end_environment_alone() {
        // \end{document} must be stripped, but \end{equation} / \end{document of love}
        // (none of those should match) must remain. The check uses
        // starts_with("\\end{document}") with the trailing `}` — anything else stays.
        let src = "\\end{equation}\n\\end{align}\n\\end{document}\n";
        let out = strip_document_scaffolding(src);
        assert!(out.contains("\\end{equation}"));
        assert!(out.contains("\\end{align}"));
        assert!(!out.contains("\\end{document}"));
    }

    #[test]
    fn test_build_canonical_image_index_prefers_png_over_jpg() {
        let d = tempfile::tempdir().unwrap();
        let illus = d.path().join("illus");
        let v516 = d.path().join("v516");
        std::fs::create_dir_all(&illus).unwrap();
        std::fs::create_dir_all(&v516).unwrap();
        std::fs::write(illus.join("12-flower-of-life.png"), b"").unwrap();
        std::fs::write(v516.join("12-flower-of-life.jpg"), b"").unwrap();
        std::fs::write(v516.join("E-lexicon.jpg"), b"").unwrap();
        std::fs::write(illus.join("not-a-chapter.png"), b"").unwrap(); // ignored: prefix invalid
        let idx = build_canonical_image_index(&illus, &v516);
        assert_eq!(idx.get("12"), Some(&"12-flower-of-life.png".to_string()));
        assert_eq!(idx.get("E"), Some(&"E-lexicon.jpg".to_string()));
        assert_eq!(idx.get("not"), None);
    }
}
