...
- **Root cause**: TerminalTabView still used `/bin/zsh -c` for arbitrary commands; clade-build and build.sh hardcoded `/Users/playra/BrowserOS/trios`; agents and skills contained emoji, arrows, and em-dashes that violated L3 PURITY.
...
- **Issue**: #T27-EPIC-001
- **Agents**: t27-creator, t27-verifier, t27-experience
- **Root cause**: Every Rust ring and `BR-OUTPUT/ProjectPaths.swift` hardcoded `/Users/playra/BrowserOS/trios` as `TRIOS_ROOT` fallback, blocking multi-machine/CI deployment and leaking developer identity. Runtime state (e2e logs, rollback snapshots, dev sandboxes) lived in `/tmp`.
